...
def auto_find_and_plot_best_segment():
    ...
    END_SEARCH = len(data_map_pos['MLP']) - WINDOW_SIZE
    ...
    for i in range(START_SEARCH, END_SEARCH):
        ...
        # [核心] 检查是否严格满足位置排序要求
        if (rmses['MLP'] < rmses['GRU'] and
                rmses['GRU'] < rmses['CNN-LSTM'] and
                rmses['CNN-LSTM'] < rmses['CNN']):

            # [核心] 计算“分离度分数” (基于位置)
            gap1 = rmses['GRU'] - rmses['MLP']
            gap2 = rmses['CNN-LSTM'] - rmses['GRU']
            gap3 = rmses['CNN'] - rmses['CNN-LSTM']

            # 以最小间距作为得分
            score = min(gap1, gap2, gap3)
    ...
    # 定义样式
    styles = {
        'MLP': {'c': '#006400', 'ls': '-', 'mk': '+', 'lw': 2.0, 'label': 'MLP'},
        'GRU': {'c': '#9467bd', 'ls': '--', 'mk': 'o', 'lw': 1.5, 'label': 'GRU'},
        'CNN-LSTM': {'c': '#ff7f0e', 'ls': '-.', 'mk': 's', 'lw': 1.5, 'label': 'CNN-LSTM'},
        'CNN': {'c': '#d62728', 'ls': ':', 'mk': '^', 'lw': 1.5, 'label': 'CNN'}
    }

    plot_order = ['CNN', 'CNN-LSTM', 'GRU', 'MLP']
    ...
    bar_order = ['MLP', 'GRU', 'CNN-LSTM', 'CNN']
