...
# 风格配置
STYLE_CONFIG = {
    # NN-IMM (Best/Proposed): 深绿, 实线, 加号 -> 主角
    'NN-IMM': {
...
DISPLAY_ORDER = ['NN-IMM', 'bayesOnline']
...
def auto_find_and_plot_best_segment():
    ...
    print(f"\n>>> 开始搜索 NN-IMM < bayesOnline 的最佳片段 (WinSize={WINDOW_SIZE})...")

    for i in range(START_SEARCH, END_SEARCH):
        segs = {k: v[i: i + WINDOW_SIZE] for k, v in data_map_pos.items()}
        rmses = {k: np.sqrt(np.mean(v ** 2)) for k, v in segs.items()}

        # [核心逻辑] NN-IMM < bayesOnline
        if 'NN-IMM' in rmses and 'bayesOnline' in rmses:

            if rmses['NN-IMM'] < rmses['bayesOnline']:

                # 分数 = 差距 (差距越大越好)
                gap = rmses['bayesOnline'] - rmses['NN-IMM']
    ...
    if best_idx == -1:
        print("[失败] 未找到 NN-IMM 优于 bayesOnline 的片段。")
        print("建议：检查数据，或确认 NN-IMM 确实比 bayesOnline 更好。")
        best_idx = END_SEARCH - 1
        print(f"-> 默认显示最后一段: {best_idx}")

    print(f"\n>>> 找到最佳片段! 起始帧: {best_idx}")
    print(f"    分离度得分 (Gap): {best_score:.5f}")
    if best_metrics:
